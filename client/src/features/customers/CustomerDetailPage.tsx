...
export default function CustomerDetailPage() {
  ...
  const [newCustomDocs, setNewCustomDocs] = useState<Array<{ id: string; documentName: string; startDate: string; endDate: string }>>([]);

  const addCustomDocToNewVehicle = () => {
    setNewCustomDocs((prev) => [
      ...prev,
      { id: crypto.randomUUID(), documentName: '', startDate: '', endDate: '' },
    ]);
  };

  const removeCustomDocFromNewVehicle = (docId: string) => {
    setNewCustomDocs((prev) => prev.filter((d) => d.id !== docId));
  };

  const updateCustomDocInNewVehicle = (
    docId: string,
    field: 'documentName' | 'startDate' | 'endDate',
    value: string
  ) => {
    setNewCustomDocs((prev) =>
  ...
  const [documentToDelete, setDocumentToDelete] = useState<DocumentWithStatus | null>(null);
  ...
  const handleConfirmDeleteDocument = async () => {
    ...
    try {
      ...
      setDeleteDocumentError(err?.response?.data?.error || 'Failed to delete document.');
    } finally {
  ...
  const handleAddVehicleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const customDocs = newCustomDocs
      ...
      await vehicleApi.create(customer.id, {
        ...
        documents: customDocs.length > 0 ? customDocs : undefined,
      });
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-2">
          {/* Active Vehicle Details & Management */}
          {activeVehicle ? (
            <>
              ...
              {/* Documents & Renewal History Card */}
                        ...
                        <div key={doc.id} className="p-4 hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition-colors flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <StatusBadge
                              ...
                              daysRemaining={
                                doc.daysRemaining ??
                                Math.ceil((new Date(doc.endDate).getTime() - new Date().setHours(0, 0, 0, 0)) / (1000 * 60 * 60 * 24))
                              }
                            />
                            <button
                              type="button"
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/50 transition-colors cursor-pointer flex items-center gap-1"
            ...
            {/* All Registered Vehicles (Vertical List Below Current Documents) */}
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Select a vehicle to view its details and current documents
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-400 border border-dashed border-indigo-300 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 cursor-pointer transition-colors"
                  >
                  ...
                  {vehiclesList.map((v, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-3 sm:p-3.5 rounded-xl text-xs transition-all border cursor-pointer text-left ${
                          isSelected
                            ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-500/20 ring-2 ring-indigo-500/30'
                            : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:border-slate-300 hover:bg-slate-50/80 dark:hover:bg-slate-800/50'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-mono text-sm font-bold block">{v.vehicleNumber}</span>
                            {v.notes && (
                              <span className={`text-[11px] block truncate max-w-[180px] sm:max-w-xs ${
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5 flex-wrap justify-end">
                          ...
                          {isSelected && (
                            <span className="text-white font-bold text-xs hidden sm:inline-block pl-1">
                              &bull; Active
                            </span>
                          )}
                        </div>
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400">
                    Custom Documents (Optional)
                  </label>
                  <button
                    type="button"
                    onClick={addCustomDocToNewVehicle}
                    className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 flex items-center gap-1.5 py-1 px-2.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-950/40 transition-colors cursor-pointer"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                  ...
                  >
                    <div className="w-full sm:w-1/3">
                      ...
                      <input
                        ...
                        onChange={(e) => updateCustomDocInNewVehicle(doc.id, 'documentName', e.target.value)}
                    ...
                    <div className="w-full sm:w-1/4">
                      ...
                      <input
                        type="date"
                        value={doc.startDate}
                        onChange={(e) => updateCustomDocInNewVehicle(doc.id, 'startDate', e.target.value)}
                    ...
                    <div className="w-full sm:w-1/4">
                      ...
                      <input
                        type="date"
                        value={doc.endDate}
                        onChange={(e) => updateCustomDocInNewVehicle(doc.id, 'endDate', e.target.value)}
                    ...
                    <button
                      type="button"
                      onClick={() => removeCustomDocFromNewVehicle(doc.id)}
                      className="mt-4 sm:mt-0 p-1.5 rounded-lg text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            ...
            <div className="text-center space-y-1.5">
              ...
              <p className="text-xs text-slate-500">
                Are you sure you want to permanently delete this {documentToDelete.documentName} document for vehicle{' '}
